...
updatedAt: "Mon Nov 20 2023 22:32:40 GMT+0000 (Coordinated Universal Time)"
---
...
The `Freeze` mutation is a powerful tool that enables the temporary suspension of activities related to a collection, token, collection account, or token account. This freezing action effectively prevents any further transactions or modifications, such as transfers or updates, from occurring. The primary purpose of freezing is to provide administrative control, enhance security, investigate suspicious activities, or respond to unforeseen events that require the temporary halt of operations.
...
- **Security and Oversight**: You can use it to enhance the security of assets in the event of suspicious activities or during an investigation. Freezing prevents any further transactions that could compromise the assets.
- **Administrative Control**: It provides a means for administrative actions on assets, such as during maintenance or upgrades, allowing you to pause activities temporarily to make necessary adjustments.
- **Emergency Response**: In unforeseen events that require an immediate response to prevent harm or loss, you can execute the freeze operation to halt all activities and protect the assets from potential damage.
...
The `Thaw` mutation is a vital operation that allows you to unfreeze a previously frozen collection or token. Freezing temporarily prevents any transactions or modifications, such as transfers, from occurring. Thawing reverses this action, restoring the collection or token's ability to be traded, transferred, or updated.
...
- **Resuming Transactions**: You can unfreeze tokens to allow trading, transferring, or updating activities to continue.
...
- **You** can use the `OperatorTransferToken` mutation in various scenarios, including:
  - **Marketplaces**: When users want to transfer assets to others within a digital marketplace, operators can facilitate the transfer on their behalf.
