...
:::tip Token ID Structure [Best Practices](/docs/tokenid-structure)
...
- **Attributes - ** Set the token details which are details stored in pairs, like a title and its content. Certain attributes, such as the `URI`, `name`, and `description`, have special roles that are understood by many platforms and marketplaces. If you're new, simply link to a JSON file that lists all the token's details. Make sure to check out the [Metadata Standard](doc:metadata-standard) page.
...
Once your token is created, lets give it a new look by [Adding Metadata](doc:adding-metadata)
